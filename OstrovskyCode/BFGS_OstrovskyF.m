function [u0min, cmin, Tmin, Gmin, niter, info] = BFGS_OstrovskyF(q, xfac, varargin)
...
% Input
% -----
% * q: Initial descent variables [u0; c; T] of length d (real). For L
...
function Gk = wrapperG(qk) %Nested functions can use global variables
        [Gk, ~] = IntegrateOstrovskyF(qk, xfac, M, J, Symm, Constraints, pandA);     
end
Phi.delG = @(qk) wrapperdelG(qk);
function delGk = wrapperdelG(qk)
        [~, delGk] = IntegrateOstrovskyF(qk, xfac, M, J, Symm, Constraints, pandA);
end
...
I = eye(d);
Hk = I;
[Gk, delGk] = IntegrateOstrovskyF(qk, xfac, M, J, Symm, Constraints, pandA);
info.Gs = Gk;
Gmin1 = Gk+1;
%info.delGs = delGk;
info.cs = qk(d-1);
