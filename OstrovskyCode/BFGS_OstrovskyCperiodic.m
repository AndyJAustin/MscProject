function [v0min, Tmin, Gmin, niter, info] = BFGS_OstrovskyCperiodic(q, xfac, varargin)
% Implements the BFGS quasi-Newton algorithm to a functional G(u0, T)
% that is zero only when the solution u(x,t) of the Ostrovsky equation 
...
function Gk = wrapperG(qk) %Nested functions can use global variables
        [Gk, ~] = IntegrateOstrovskyCperiodic(qk, xfac, M, J, Symm, Constraints, pandA);     
end
Phi.delG = @(qk) wrapperdelG(qk);
function delGk = wrapperdelG(qk)
        [~, delGk] = IntegrateOstrovskyCperiodic(qk, xfac, M, J, Symm, Constraints, pandA);
end
...
I = eye(d);
Hk = I;
[Gk, delGk] = IntegrateOstrovskyCperiodic(qk, xfac, M, J, Symm, Constraints, pandA);
info.Gs = Gk;
Gkmin1 = Gk + 1;
%info.delGs = delGk;
info.Ts = qk(d);
