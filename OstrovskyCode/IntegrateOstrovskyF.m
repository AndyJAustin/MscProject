function [Gk, delGk] = IntegrateOstrovskyF(qk, xfac, M, J, Symm, Constraints, pandA)
...
if isequal(Symm, true)
    ...
    [dGduF, ~, ~] = AIOstrovskyUneven(u0, u_lj, u_lj_t, T, c, M, J, x);
end
...
delGk = zeros(d,1);
if isequal(Symm, true)
    delGk(1:d-2) = dGduF(1:L/2+1); 
    delGk(d-1) = dGdc; delGk(d) = dGdT;
else
    delGk(1:d-2) = dGduF; 
    delGk(d-1) = dGdc; delGk(d) = dGdT;
end
