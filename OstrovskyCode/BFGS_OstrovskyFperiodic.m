function [u0min, Tmin, Gmin, niter, info] = BFGS_OstrovskyFperiodic(q, xfac, varargin)
...
% periodic x domain, we perform the integration from a fixed frame and
% remove the translation term of the Ostrovsky eqaution, negating any need
% for a speed variable c. The time taken to perform each iteration is
...
function Gk = wrapperG(qk) %Nested functions can use global variables
        [Gk, ~] = IntegrateOstrovskyFperiodic(qk, xfac, M, J, Symm, Constraints, pandA);     
end
Phi.delG = @(qk) wrapperdelG(qk);
function delGk = wrapperdelG(qk)
        [~, delGk] = IntegrateOstrovskyFperiodic(qk, xfac, M, J, Symm, Constraints, pandA);
end
...
I = eye(d);
Hk = I;
[Gk, delGk] = IntegrateOstrovskyFperiodic(qk, xfac, M, J, L, Symm, Constraints, pandA);
info.Gs = Gk;
Gmin1 = Gk+1;
%info.delGs = delGk;
info.Ts = qk(d);
k = 0;
while norm(delGk) > epsilon && k < MaxIter && Gmin1 - Gk > 0
    tic
    ...
    delGkmin1 = delGk;
    Gmin1 = Gk;
    [Gk, delGk] = IntegrateOstrovskyFperiodic(qk, xfac, M, J, Symm, Constraints, pandA);
